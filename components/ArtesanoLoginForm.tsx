...
export default function ArtesanoLoginForm() {
  ...
  useEffect(() => {
    if (loading && user && profile) {
      // Verificar que el usuario tenga rol de artesano
      if (profile.role?.name === 'artesano') {
        router.push('/artesano')
        setLoading(false)
      } else {
        setError('No tienes permisos de artesano. Por favor, contacta al administrador.')
        setLoading(false)
      }
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              <form onSubmit={(e) => { e.preventDefault(); console.log('Búsqueda desde login artesano'); }} className="relative">
                <input
                  type="text"
                  placeholder="BUSCAR..."
      ...
      <div className="flex items-center justify-center px-4 py-12">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm" style={{ color: '#0f324b', opacity: 0.7 }}>
            Ingresa tus credenciales para acceder a tu panel de artesano
          </p>
        </div>
